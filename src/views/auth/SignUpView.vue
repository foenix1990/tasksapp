<template>
  <div class="signIn">
    ...
    <Input class="input" type="text" text="Имя" :model="username" v-on:valid="usernameValid = $event" v-on:change="username = $event" length="6" icon="user.svg" />
    ...
    <div class="buttons">
      <div class="row lft">
        <div class="links" @click="next(3, true)">Забыл пароль</div>
      </div>
      <div class="row rgt">
        <div class="links" @click="next(1, false)">Авторизация</div>
      </div>
    </div>
    <div class="button" @click="open" :class="[usernameValid && emailValid ? '' : 'blocked']">Отправить</div>
  </div>
</template>
